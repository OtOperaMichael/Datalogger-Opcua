...
import {NodeGroupType, type CommTableInterface} from "@/types/template.ts";
...
const isDisabled = computed(() => !loginUserStore.getIsLoggedIn || !template.communication.enable || !template.editing);

const tableData = computed<CommTableInterface>(() => {
  return template.communication.tableList[props.tableIndex] || {
    name: "",
    sampleInterval: 1000,
...
function addNode() {
  template.addTagName('communication', props.tableIndex);
}

function removeNode(index: number) {
  template.removeTagName('communication', props.tableIndex, index);
}
...
watch(
  () => [tableData.value.nodeList[0]?.nodeId, tableData.value.nodeGroupType],
  () => {
